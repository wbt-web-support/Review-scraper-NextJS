...
const ChartStyle = ({ id, config: chartConfigProp }: { id: string; config: ChartConfig }) => {
   const validColorEntries = Object.entries(chartConfigProp).filter(
    ([_key, itemConfig]) => itemConfig.theme || itemConfig.color 
  );
  if (!validColorEntries.length) {
    return null;
  }
    return (
        <style
